...
static int lposix_pushresult(lua_State *L, int i, const char *info)
{
	if (i != -1)
	{
		lua_pushnumber(L, i);
...
static int Pmkdir(lua_State *L)			/** mkdir(path) */
{
	...
	return lposix_pushresult(L, mkdir(path, 0777), path);
}
...
{
	const char *path = luaL_checkstring(L, 1);
	return lposix_pushresult(L, chdir(path), path);
}
...
{
	const char *path = luaL_checkstring(L, 1);
	return lposix_pushresult(L, rmdir(path), path);
}
...
{
	const char *path = luaL_checkstring(L, 1);
	return lposix_pushresult(L, unlink(path), path);
}
...
static int Plink(lua_State *L)			/** link(oldpath,newpath) */
{
	...
	return lposix_pushresult(L, link(oldpath, newpath), NULL);
}
...
static int Psymlink(lua_State *L)		/** symlink(oldpath,newpath) */
{
	const char *oldpath = luaL_checkstring(L, 1);
	const char *newpath = luaL_checkstring(L, 2);
	return lposix_pushresult(L, symlink(oldpath, newpath), NULL);
}
...
static int Paccess(lua_State *L)		/** access(path,[mode]) */
{
	...
	return lposix_pushresult(L, access(path, mode), path);
}
...
{
	const char *path = luaL_checkstring(L, 1);
	return lposix_pushresult(L, mkfifo(path, 0777), path);
}
...
{
	return lposix_pushresult(L, fork(), NULL);
}
...
static int Pwait(lua_State *L)			/** wait([pid]) */
{
	...
	return lposix_pushresult(L, waitpid(pid, NULL, 0), NULL);
}
...
static int Pkill(lua_State *L)			/** kill(pid,[sig]) */
{
	...
	return lposix_pushresult(L, kill(pid, sig), NULL);
}
...
static int Pputenv(lua_State *L)		/** putenv(string) */
{
	...
	char *e=malloc(++l);
	return lposix_pushresult(L, (e==NULL) ? -1 : putenv(memcpy(e,s,l)), s);
}
...
static int Psetenv(lua_State *L)		/** setenv(name,value,[over]) */
{
	...
	return lposix_pushresult(L, setenv(name,value,overwrite), name);
}
...
static int Pchmod(lua_State *L)			/** chmod(path,mode) */
{
	...
	return lposix_pushresult(L, chmod(path, mode), path);
}
...
static int Pchown(lua_State *L)			/** chown(path,uid,gid) */
{
	...
	return lposix_pushresult(L, chown(path, uid, gid), path);
}
...
static int Putime(lua_State *L)			/** utime(path,[mtime,atime]) */
{
	...
	return lposix_pushresult(L, utime(path, &times), path);
}
...
{
	return lposix_pushresult(L, setuid(mygetuid(L, 1)), NULL);
}


static int Psetgid(lua_State *L)		/** setgid(name or id) */
{
	return lposix_pushresult(L, setgid(mygetgid(L, 1)), NULL);
}
